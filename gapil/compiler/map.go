...
const fullBucket = uint64(1)
const usedBucket = uint64(2)
...
func (c *compiler) buildMapType(t *semantic.Map) {
	...
	c.build(contains, func(s *scope) {
		...
		s.ForN(capacity, func(it *codegen.Value) *codegen.Value {
			...
			s.If(c.equal(s, valid, s.Scalar(emptyBucket)), func() {
				s.Return(s.Scalar(false))
			})
			s.If(c.equal(s, valid, s.Scalar(fullBucket)), func() {
	...
	c.build(index, func(s *scope) {
		...
		s.ForN(capacity, func(it *codegen.Value) *codegen.Value {
			...
			return s.Not(c.equal(s, valid, s.Scalar(emptyBucket)))
		})
		...
		s.If(addIfNotFound, func() {
			...
			getStorageBucket := func(k, h, table, tablesize *codegen.Value) *codegen.Value {
				...
				s.ForN(tablesize, func(it *codegen.Value) *codegen.Value {
					...
					notFound := c.equal(s, valid, s.Scalar(fullBucket))
					return notFound
				})
			...
			s.If(resize.Load(), func() {
				...
				s.IfElse(elements.IsNull(), func() {
					...
					s.ForN(capacity, func(it *codegen.Value) *codegen.Value {
						elements.Index(it, "used").Store(s.Scalar(emptyBucket))
						return nil
					})
				}, /* else */ func() {
					...
					s.ForN(newCapacity, func(it *codegen.Value) *codegen.Value {
						newElements.Index(it, "used").Store(s.Scalar(emptyBucket))
						return nil
					})
					...
					s.ForN(capacity, func(it *codegen.Value) *codegen.Value {
						...
						s.If(c.equal(s, valid, s.Scalar(fullBucket)), func() {
							...
							h := c.hashValue(s, t.KeyType, k)
							bucket := getStorageBucket(k, h, newElements, newCapacity)
							newElements.Index(bucket, "k").Store(k)
							newElements.Index(bucket, "v").Store(v)
							newElements.Index(bucket, "used").Store(s.Scalar(fullBucket))
						})
						return nil
					})
					c.free(s, elements)
			...
			capacity := capacityPtr.Load()
			elements := elementsPtr.Load()
			bucket := getStorageBucket(k, h, elements, capacity)
			elements.Index(bucket, "k").Store(k)
			elements.Index(bucket, "used").Store(s.Scalar(fullBucket))
		...
		s.ForN(capacity, func(it *codegen.Value) *codegen.Value {
			...
			s.If(c.equal(s, valid, s.Scalar(fullBucket)), func() {
				...
				s.If(found, func() {
					...
					elPtr.Index(0, "used").Store(s.Scalar(usedBucket))
					count := countPtr.Load()
