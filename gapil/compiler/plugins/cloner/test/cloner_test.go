...
func TestCloneBasicTypes(t *testing.T) {
	...
	assert.For(ctx, "arena").That(a1.Stats()).Equals(a2.Stats())
...
func TestCloneMaps(t *testing.T) {
	...
	orig := MakeMapsʳ(a1)
	orig.SetA(NewU8ːu8ᵐ(a1).Add(1, 10).Add(2, 20).Add(3, 30))
	orig.SetB(NewStringːBasicTypesʳᵐ(a1).Add("foo", MakeBasicTypesʳ(a1)))
...
func TestCloneArrays(t *testing.T) {
	...
	orig := MakeArraysʳ(a1)
	orig.SetA(NewU8ː4ᵃ(a1, 10, 20, 30, 40))
	orig.SetB(NewBasicTypesː4ᵃ(a1, MakeBasicTypes(a1), MakeBasicTypes(a1), MakeBasicTypes(a1), MakeBasicTypes(a1)))
