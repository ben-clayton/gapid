...
context* create_context(gapil_module* m, arena* a) {
  return m->create_context(a);
}

void destroy_context(gapil_module* m, context* ctx) { m->destroy_context(ctx); }
...
void call(context* ctx, gapil_module* m, cmd_data_t* cmds, uint64_t count,
          uint64_t* res) {
...
void set_callbacks(callbacks* go_cbs) {
  ...
  gapil_runtime_callbacks cbs = go_callbacks;
  cbs.call_extern = &call_extern;
  cbs.apply_reads = &apply_reads;
  cbs.apply_writes = &apply_writes;
