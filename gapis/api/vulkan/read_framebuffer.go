...
func postImageData(ctx context.Context,
	...
	stagingImageCreateInfo := NewVkImageCreateInfo(a,
		...
		vkFormat,                     // format
		NewVkExtent3D(a, reqWidth, reqHeight, 1), // extent
		1, // mipLevels
		1, // arrayLayers
	...
	bufferImageCopy := NewVkBufferImageCopy(a,
		...
		NilVkOffset3D,                            // imageOffset
		NewVkExtent3D(a, reqWidth, reqHeight, 1), // imageExtent
	)
	...
	imageBlit := NewVkImageBlit(a,
		...
		NewVkOffset3Dː2ᵃ(a, // dstOffsets
			NilVkOffset3D,
			NewVkOffset3D(a, int32(reqWidth), int32(reqHeight), 1),
		),
	)
