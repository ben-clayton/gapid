	...
	for i, devs := range devices {
		...
		for _, device := range devs {
			...
			sb.write(sb.cb.VkGetPhysicalDeviceQueueFamilyProperties(
				...
				NewVkQueueFamilyPropertiesᵖ(sb.MustUnpackWriteMap(pd.QueueFamilyProperties().Len()).Ptr()),
			))
		}
	}
}
...
func (sb *stateBuilder) transitionImage(image ImageObjectʳ,
	...
	sb.write(sb.cb.VkCmdPipelineBarrier(
		...
		memory.Nullptr,
		1,
		sb.MustAllocReadData(NewVkImageMemoryBarrier(sb.ta,
...
func (sb *stateBuilder) createSwapchain(swp SwapchainObjectʳ) {
	...
	sb.write(sb.cb.VkCreateSwapchainKHR(
		...
		sb.MustAllocReadData(NewVkSwapchainCreateInfoKHR(sb.ta,
			...
			NewU32ᶜᵖ(sb.MustUnpackReadMap(swp.Info().QueueFamilyIndices().Len()).Ptr()), // pQueueFamilyIndices
